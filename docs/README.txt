DLSS5 工具（GPU 直通版）
========================
导入视频 → 实时预览(原图/DLSS/对比) → 调风格/强度/本地色调/本地结构 →
选编码器 → 导出 DLSS 视频(带音轨)。
...
【目录结构】(2026-09 重组)
  ...
  ★ DirectShow 滤镜(dlssnr_dshow.cpp / dlssnr_panel.py / register_filter.bat)
    已拆分到【独立仓库 dlssnr-filter】，不在本仓库内。
...
【实时控制面板 dlssnr_panel.py】(随滤镜一起拆分到 dlssnr-filter 仓库)
  直接运行(或 pythonw 无控制台):     python dlssnr_panel.py
