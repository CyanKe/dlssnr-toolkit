...
【已知限制】
  ...
  - 宿主 dlssnr2_* / vfx / truehdr 内部【没有任何锁】，同一进程里同一时刻
    只能有一个线程进去。界面用一把 `_gpu_claim` 保证这件事(见「已修复的坑」15)；
    自己写脚本直接调 dlss_engine.engine2 时也要自己保证「预览」和「导出」
    不同时进，否则会互相释放纹理，表现为取回失败甚至 access violation。
...
【已修复的坑】(踩过的,别再踩)
  ...
  15. 【导出中动预览会拆掉正在跑的会话】engine2 是进程级单例，而 dlssnr2_*
     内部【没有任何锁】。预览和导出工作线程同时进去就会互相踩:
     预览的 process() 会把导出已提交、还没取回的那一帧 DrainCore 掉；
     改「预览上限」更狠 —— 预览会把引擎重新 ensure 到新尺寸，
     dlssnr2_init 走 ReleaseFeatureAndTextures()，把导出线程正在用的
     feature / 纹理 / 槽位全部释放。
     实测(真实 App + 真实队列 + 真实 ffmpeg，1080p 源，导出中每 40ms 改一次
     「预览上限」):
       改前: 日志出现 `[DLSS] exception: access violation writing 0x...`
             与 `帧 0: 提交失败，终止导出`，三个任务输出 2/0/0 帧；
             引擎层单独复现同样致命: 预览线程碰引擎时导出线程报
             `帧 0: 取回失败`，或直接 OSError: access violation reading 0x0。
             注意 ffmpeg 对截断的 rawvideo 仍然退出 0，所以损坏的任务还显示
             「完成」 —— 比报错更危险。
       改后: 3 个队列任务全部「完成」，输出各 60/60 帧且可解码；
             期间 55 次「预览上限」改动无一命中。
     现在由一把 `_gpu_claim` (threading.Lock) 做【唯一占用声明】:
       - 工作线程在 _export_job_impl 里持锁跑完整个任务。不用「主线程收到
         task_start 再置位」的普通标志: 那个标志最多晚一个轮询周期(150ms)，
         而刚点开始/任务交接的那一小段正好是出问题的窗口。
       - 预览只 try-acquire，失败就显示原图，UI 永远不会等导出。
       - _refresh_dlss 在占用期间不推参数(否则会把导出的参数悄悄改掉)。
       - TrueHDR 的可用性探测(它会启动 NGX 核心)同样避开占用期，
         首次探测推迟到空闲时。
       - 任务结束后 _gpu_forget_size() 清掉尺寸缓存，预览会按自己的尺寸
         重新 ensure 并置 reset，不会拿着导出留下的状态继续跑。
     「暂停队列」的语义: 任务内的暂停仍然占着 GPU(已提交的帧不能丢给预览)，
     所以日志改成实话「任务占用 GPU 期间预览显示原图」；
     只有任务之间/开始前暂停，预览才能真正回来。
  16. 【队列进度泵被自己的转发器打死】_run_queue_task 曾把 self._prog_q
     换成只实现 put() 的转发对象，而主线程的 _poll_queue 同一时刻还在
     self._prog_q.get_nowait() → AttributeError 冒泡出 Tk 回调 → 泵不再重排
     → 之后的 task_done / queue_done 永远没人处理: 任务卡在「转换中」、
     开始按钮永久禁用、队列再也不动。同一处转发器上的
     `self._queue_stop`(那个 self 是转发器，不是 App)让工作线程在第一个任务
     结束时就崩掉。
     现在转发器是【参数 sink】而不是替换属性，主线程永远读真正的队列；
     工作线程里的任务异常也被捕获并记日志，不会再让整条队列挂死。
     顺带把「有错误记录」(取回失败/用户取消/截断)也算失败: 以前 ffmpeg 对
     截断输入退出 0，会把空文件报成「完成」。
