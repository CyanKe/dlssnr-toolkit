...
【功能】
  ...
  【多层 DLSSNR】(参数卡里的"层"一行)
    ...
    执行: 每帧对同一帧依次跑完所有层 —— 第 1 层的输出喂给第 2 层，
          ...
          双击任务载回界面时层链一起恢复。
    代价: 每层都是一次完整的"上传→推理→回读"，所以耗时约为单层的 N 倍
          (1080p 单层约 9.5 ms，三层约 28 ms)。多层时导出不再走双缓冲流水
          (多层之间有依赖，重叠不起来)，日志会写明"每帧依次执行 N 层"。
    重要: 各层【共用同一个 NGX feature 和时域历史】。也就是说这是"同一个网络
          每帧跑 N 次、每次换一组参数"，不是 N 个互相独立、各有自己历史的滤波器。
          第一层的时域缓冲会被第二次调用继续更新。要真正的独立历史，
          需要宿主里建多个 feature(见 dlssnr_host2.cpp)，那是另一件事。
          另外这是同一个非线性、带时域的网络串联，不是线性滤波叠加，
          效果只能拿真实素材对比(HDR/RTX 各阶段同理)。
...
【文件夹内容】
  ...
  dlssnr_host2.cpp    新宿主源码(BGR进/RGBA出，GPU 直通 + 双缓冲)
