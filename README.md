...
Windows 上的离线视频处理工具：导入 → 实时预览（原图 / DLSS / 对比）→ 调参 → 选编码器 → 导出带音轨的成片。
...
> 本仓库负责**离线导出**；滤镜负责**播放器里实时处理**。两者共用同一个引擎。
...
DirectShow 滤镜因为要额外做 `NV12 → BGR24 → 引擎 → BGR24 → NV12` 的往返转换，
同样的 1920×1440 下约为 14.3 ms —— 滤镜那边的数据见 dlssnr-filter 仓库。
...
引擎的二进制接口（导出函数签名、缓冲区契约）是两者之间的**唯一约定**，
已记录在 dlssnr-filter 仓库的 `docs/ENGINE_INTERFACE.md`。
...
│   └── *.dll               ★ 本机运行库，不进仓库
├── src/                    源码
│   ├── dlssnr_host2.cpp    DLSSNR 宿主（D3D12，GPU 直通）★ 滤镜仓库也用它
...
| 功能 | 说明 |
| --- | --- |
| 显示模式 | 原图 / DLSS / 对比（可拖分界线） |
| 播放 | 按视频帧率逐帧实时处理 |
| 预览处理上限 | 不限制 / 720p / 1080p（默认）/ 1440p —— 只影响预览，导出始终用原始分辨率 |
| DLSS 参数 | 风格（默认·自然·电影·风格3）、强度、局部色调、局部结构 |
...
> **给 dlssnr-filter 用的就是 `dlssnr_host2.dll`。** 如果你想在播放器里用滤镜，
> 编译出这个 DLL 后放进滤镜仓库的 `app\` 目录即可 —— 滤镜仓库本身不需要任何 NVIDIA SDK。
